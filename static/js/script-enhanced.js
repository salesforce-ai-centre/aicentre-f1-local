document.addEventListener('DOMContentLoaded', (event) => {
    ...
    // Performance optimization: Smart DOM updates
    const domCache = {
        lastValues: new Map(),
    ...
    function updateTyreVisualization(tyreData) {
        ...
        tyres.forEach(tyre => {
            ...
            // Add critical wear highlighting (>70%)
            if (wear > 70) {
                tyreElement.classList.add('critical-wear');
            }

            // Add critical temp highlighting (>105°C or <80°C)
            if (temp > 105 || temp < 80) {
    ...
    function updateGForceMeter(lateral, longitudinal) {
        ...
        // Update text values
        domCache.updateElement(document.getElementById('gLateral'),
            `${lateral.toFixed(1)}g`, null, 50);
        domCache.updateElement(document.getElementById('gLongitudinal'),
            `${longitudinal.toFixed(1)}g`, null, 50);
    }
    ...
    function updateDashboard(data) {
        ...
        // Session info
        domCache.updateElement(document.getElementById('driverName'), data.driverName || '-');
        domCache.updateElement(document.getElementById('trackName'), data.trackName || '-');
        domCache.updateElement(document.getElementById('position'), data.position || '-');
        domCache.updateElement(document.getElementById('currentLap'), data.currentLap || '-');
        ...
        // Lap times
        domCache.updateElement(document.getElementById('currentLapTime'),
            formatLapTime(data.currentLapTime), null, 100);
        domCache.updateElement(document.getElementById('lastLapTime'),
        ...
        domCache.updateElement(document.getElementById('speedValue'),
            `${Math.round(data.speed || 0)} km/h`, null, 50);
        domCache.updateElement(document.getElementById('rpmValue'),
            Math.round(data.engineRPM || 0), null, 50);

        // Gear
        domCache.updateElement(document.getElementById('gear'),
        ...
        domCache.updateElement(document.getElementById('steeringValue'),
            `${steeringDegrees.toFixed(0)}°`, null, 50);

        // Tyres with critical highlighting
        if (data.tyresWear) {
            const tyreData = {
                frontleftWear: data.tyresWear[2] || 0,
                frontrightWear: data.tyresWear[3] || 0,
                rearleftWear: data.tyresWear[0] || 0,
                rearrightWear: data.tyresWear[1] || 0,
                frontleftTemp: data.tyresSurfaceTemperature?.[2] || 0,
                frontrightTemp: data.tyresSurfaceTemperature?.[3] || 0,
                rearleftTemp: data.tyresSurfaceTemperature?.[0] || 0,
                rearrightTemp: data.tyresSurfaceTemperature?.[1] || 0
            };

            updateTyreVisualization(tyreData);

            // Update individual tyre values
            domCache.updateElement(document.getElementById('tyreFLWear'),
                `${Math.round(tyreData.frontleftWear)}%`);
            domCache.updateElement(document.getElementById('tyreFRWear'),
                `${Math.round(tyreData.frontrightWear)}%`);
            domCache.updateElement(document.getElementById('tyreRLWear'),
                `${Math.round(tyreData.rearleftWear)}%`);
            domCache.updateElement(document.getElementById('tyreRRWear'),
                `${Math.round(tyreData.rearrightWear)}%`);

            domCache.updateElement(document.getElementById('tyreFLTemp'),
                `${Math.round(tyreData.frontleftTemp)}°C`);
            domCache.updateElement(document.getElementById('tyreFRTemp'),
                `${Math.round(tyreData.frontrightTemp)}°C`);
            domCache.updateElement(document.getElementById('tyreRLTemp'),
                `${Math.round(tyreData.rearleftTemp)}°C`);
            domCache.updateElement(document.getElementById('tyreRRTemp'),
                `${Math.round(tyreData.rearrightTemp)}°C`);
        }

        domCache.updateElement(document.getElementById('tyreCompound'),
        ...
        domCache.updateElement(document.getElementById('ersPercent'),
            `${Math.round((data.ersStoreEnergy || 0) / 4000000 * 100)}%`);
        domCache.updateElement(document.getElementById('ersDeployMode'),
        ...
        // Fuel info
        domCache.updateElement(document.getElementById('fuelLoad'),
            `${(data.fuelInTank || 0).toFixed(1)}kg`);
        domCache.updateElement(document.getElementById('fuelMix'),
            data.fuelMix || 'Standard');
        domCache.updateElement(document.getElementById('fuelLaps'),
